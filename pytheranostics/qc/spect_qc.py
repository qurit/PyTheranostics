...
class SPECTQC(QC):
    ...
    def check_camera_parameters(self, ds, projs=True):
        ...
        if projs:
            ...
            if (
                number_projections
                == self.isotope_dic["raw"]["n_proj"][
                    camera_manufacturer.lower().split(" ")[0]
                ]
            ):
                self.append_to_summary(
                    f"NUMBER OF PROJECTIONS: {number_projections} of {self.isotope_dic['raw']['n_proj'][camera_manufacturer.lower().split(' ')[0]]}\t         OK\n\n"
                )
            else:
                self.append_to_summary(
                    f"NUMBER OF PROJECTIONS: {number_projections} of {self.isotope_dic['raw']['n_proj'][camera_manufacturer.lower().split(' ')[0]]}\t         VERIFY\n\n"
                )
            ...
            if round(duration) == self.isotope_dic["raw"]["duration_per_proj"]:
                self.append_to_summary(
                    f"DURATION PER PROJECTION: {round(duration)} seconds\t       OK\n\n"
                )
            else:
                self.append_to_summary(
                    f'DURATION PER PROJECTION: {duration} seconds (should be {self.isotope_dic["planar"]["duration"]} seconds)\t       MISMATCH \n\n'
                )
            ...
            if [rows, cols] == self.isotope_dic["raw"]["matrix"]:
                ...
                self.append_to_summary(
                    f'MATRIX SIZE: {rows} x {cols} (should be {self.isotope_dic["spect"]["matrix"][0]} x {self.isotope_dic["spect"]["matrix"][1]}\tMISMATCH\n\n'
                )

            # check detector motion
            if ds.TypeOfDetectorMotion == self.isotope_dic["raw"]["detector_motion"]:
                self.append_to_summary(
                    f"DETECTOR MOTION: {self.isotope_dic['raw']['detector_motion']}\t       OK\n\n"
                )
            else:
                self.append_to_summary(
                    f"DETECTOR MOTION: {ds.TypeOfDetectorMotion} (should be {self.isotope_dic['raw']['detector_motion']})\t        MISMATCH \n\n"
                )
            ...
            try:
                if set(self.isotope_dic["spect"]["corrections"]).issubset(
                    set(list(ds.CorrectedImage.split())).intersection(
                        set(self.isotope_dic["spect"]["corrections"])
                    )
                ):
                    self.append_to_summary(
                        f"CORRECTIONS APPLIED: {self.isotope_dic['spect']['corrections']}\t                   OK\n\n"
                    )
                else:
                    self.append_to_summary(
