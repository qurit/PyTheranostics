...
class QC:
        ...
        **kwargs:
        ...
        if "db_dic" in kwargs:
            ...
            if "calibration_data" in db_df.keys():
                ...
                # convert columns of date and time to datetime
                cal_forms["measurement_datetime"] = pd.to_datetime(
                    cal_forms["measurement_datetime"], format="%Y%m%d %H:%M"
                )
                ref_shipped["ref_datetime"] = pd.to_datetime(
                    ref_shipped["ref_datetime"], format="%Y%m%d %H:%M"
                )

                # only look at the center being qualified and the type of calibration necessary
                if "site_id" in kwargs["db_dic"]:
                    self.db_df["cal_data"] = cal_forms[
                        (cal_forms.site_id == site_id)
                        & (cal_forms.cal_type == cal_type)
                    ]
                    self.db_df["shipped_data"] = ref_shipped[
                        (ref_shipped.site_id == site_id)
                    ]
                else:
                    self.db_df["cal_data"] = cal_forms
                    self.db_df["shipped_data"] = ref_shipped
    ...
    def window_check(self, win_perdiff_max=2, type="planar"):
        ...
        for i in range(len(ds.EnergyWindowInformationSequence)):
            low = (
                ds.EnergyWindowInformationSequence[i]
        ...
        missing_win_keys = list(
            set(self.isotope_dic["windows_kev"].keys()) - set(win_check)
        )
        common_win_keys = set(self.isotope_dic["windows_kev"].keys()).intersection(
            set(win_check)
        )
        ...
        if type != "spect":
            if missing_win_keys:
                self.append_to_summary(
                    f"The dataset is missing a window for {missing_win_keys}.\tMISMATCH\n"
                )
            else:
                self.append_to_summary(
                    "The dataset contains all the expected energy windows.\tOK\n"
                )
        else:
            if "photopeak" in common_win_keys:
                self.append_to_summary(
                    f"The reconstructed image is for the correct photopeak, centered at {win_check['photopeak'][1]} keV.\tOK\n"
                )
        ...
        if type != "spect":
            ...
            if (perc_diff_df.abs() > win_perdiff_max).any(axis=None):
                self.append_to_summary(
                    f"There are differences in the energy window settings that are higher than {win_perdiff_max} %.\nPlease see below:\t       MISMATCH\n\n"
                )
    ...
    def print_summary(self):
        ...
        self.summary_df.style.set_properties(
            subset=[cols], **{"width": "500px"}, **{"text-align": "left"}
        ).hide()
        # print(self.summary)
    ...
    def update_db(self, syringe_name="syringe_20_mL"):
        ...
        for s in sources:
            for c in centres:
                ...
                ref_act = self.db_df["shipped_data"][
                    ...
                    & (self.db_df["shipped_data"].site_id == c)
                ]["A_ref_MBq"]
                ...
                self.db_df["cal_data"].loc[
                    (
                        (self.db_df["cal_data"].source_id == s)
                        & (self.db_df["cal_data"].site_id == c)
                    ),
                    "ref_act_MBq",
                ] = ref_act.values[0]
                self.db_df["cal_data"].loc[
        ...
        self.db_df["cal_data"]["delta_t"] = (
            self.db_df["cal_data"]["measurement_datetime"]
            - self.db_df["cal_data"]["ref_time"]
        ) / np.timedelta64(1, "D")
