...
class DosecalQC(QC):
    ...
    def check_calibration(self, accepted_percent=1.5, accepted_recovery=(97, 103)):
        ...
        self.append_to_summary(
            f"QC for Dose Calibrator Calibration of {self.isotope}:\t \n\n"
        )
        ...
        if (
            ...
            if (
                (accepted_recovery[0] <= self.db_df["cal_data"]["recovery_calculated"])
                & (
                    accepted_recovery[1]
                    >= self.db_df["cal_data"]["recovery_calculated"]
                )
                ...
                self.append_to_summary(
                    f"The reported recovery matches the calculated recovery and falls within {accepted_recovery[0]} % and {accepted_recovery[1]} %.\t            OK\n\n"
                )
            ...
            if (
                ...
                self.append_to_summary(
                    f"One or more reported recovery does not match the calculated recovery. However, the calculated recovery is within the accepted range of {accepted_recovery[0]} % and {accepted_recovery[1]} %.\t             VERIFY\n"
                )
                # self.append_to_summary(f'Please see below:\t \n\n')
            else:
                self.accepted_calibration = 3
                self.append_to_summary(
                    f"One or more reported recovery does not match the calculated recovery and the calculated recovery is out of the acceptable range of {accepted_recovery[0]} % to {accepted_recovery[1]} %.\t             FAIL\n"
                )
                # self.append_to_summary(f'Please see below:\t \n\n')
        ...
        self.accepted_calnum_df = (
            self.db_df["cal_data"][
                ["manufacturer", "model", "source_id", "final_cal_num"]
            ]
            .groupby(["manufacturer", "model", "source_id"])
            .sum()
        )
    ...
    def check_source_decay(self, accepted_percent):
        ...
        for s in sources:
            ...
            ref_act = self.db_df["shipped_data"][
                self.db_df["shipped_data"].source_id == s
            ]["A_ref_MBq"]
            ref_time = self.db_df["shipped_data"][
                self.db_df["shipped_data"].source_id == s
            ]["ref_datetime"]

            # calculate the delta time of the calibration of the shipped source and the measurement
            self.db_df["cal_data"].loc[
                self.db_df["cal_data"].source_id == s, "delta_t"
            ] = (
                self.db_df["cal_data"][self.db_df["cal_data"]["source_id"] == s][
            ...
            self.db_df["cal_data"].loc[
                self.db_df["cal_data"].source_id == s, "ref_act_MBq"
            ] = ref_act.values[0]

        self.db_df["cal_data"]["decayed_ref"] = np.nan

        # decay correct reference sources to measurement time series
        self.db_df["cal_data"].decayed_ref = self.db_df["cal_data"].apply(
            lambda row: decay_act(
                row.ref_act_MBq, row.delta_t, self.isotope_dic["half_life"]
            ),
            axis=1,
        )

        # Check that decay has been applied correctly. calculate perc_diff
        self.db_df["cal_data"]["decay_perc_diff"] = perc_diff(
            self.db_df["cal_data"]["Ae_MBq"], self.db_df["cal_data"].decayed_ref
        )

        # check recovery with the calculated decayed activity
        self.db_df["cal_data"]["recovery_calculated"] = (
            self.db_df["cal_data"].Am_MBq / self.db_df["cal_data"]["decayed_ref"] * 100
        ).round(1)

        # check if any perc_diff are higher than accepted
        if (self.db_df["cal_data"]["decay_perc_diff"].abs() > accepted_percent).any(
            axis=None
        ):
            self.accepted_calibration = 2
            self.append_to_summary(
                f"One or more of the sources decay correction are off by more than {accepted_percent} %.\t      MISMATCH\n\n"
            )
            ...
            self.append_to_summary(
                f"All the sources decay correction are within {accepted_percent} % of the expected.\t                               OK\n\n"
            )
    ...
    def check_syringe_recovery(self, syringe_name="syringe_20_mL"):
        ...
        self.db_df["cal_data"].loc[
            ...
            self.db_df["cal_data"]["Ai_syr_MBq"] - self.db_df["cal_data"]["Af_syr_MBq"]
        )
        self.db_df["cal_data"].loc[
            self.db_df["cal_data"].source_id == syringe_name, "recovery_calculated"
        ] = (
            self.db_df["cal_data"]["Am_syr_MBq"]
            / self.db_df["cal_data"]["syringe_activity_calculated"]
            * 100
        ).round(
            1
        )
