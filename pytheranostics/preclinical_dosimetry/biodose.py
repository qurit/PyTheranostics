...
class BioDose:
    ...
    def read_biodi(self, biodi_file):
        ...
        biodi["Organ"] = biodi["Organ"].replace(
            "Submandibular Glands", "Salivary Glands"
        )
        biodi["Organ"] = biodi["Organ"].replace(
            "Submandibular Gland", "Salivary Glands"
        )
        ...
        biodi = pd.concat(
            [biodi.iloc[:, : len(self.t)], biodi.iloc[:, len(self.t) :]],
            axis=1,
            keys=["%ID/g", "sigma"],
        )
    ...
    def initialize_results_df(self):
        ...
        columns = [
            "Mono-Exponential",
            "Bi-Exponential",
            "Bi-Exponential_uptake",
            "Tri-Exponential",
            "Perctage_diff - mono vs bi washout",
        ...
        for org in organlist:
            ...
            if not uptake:
                ...
                result_bi, fitted_bi = exponential_fit_lmfit(
                    ...
                    sigma=sigmas[tps_to_skip_fit:],
                    params_init={
                        "A1": result_mono.params["A1"].value * 0.6,
                        "A2": result_mono.params["A2"].value * 0.8,
                        "B1": result_mono.params["A1"].value * 0.4,
                        "B2": result_mono.params["A2"].value * 1.2,
                    },
                    bounds={
                        "A1": (0, None),
                        "A2": (decayconst, None),
                ...
                if tps_to_skip_fit == 0:
                    ...
                    trapezoid_area = (
                        (bio_data.iloc[0] + bio_data.iloc[1]) * (t[1] - t[0]) / 2
                    )
                    ...
                    if tps_to_skip_fit == 1:
                        ...
                        trapezoid_area2 = (
                            (bio_data.iloc[1] + bio_data.iloc[2]) * (t[2] - t[1]) / 2
                        )
                        area_mono = (
                            triangle_area
                            + trapezoid_area
                ...
                area_uptake = integrate.quad(
                    lambda x: biexp_fun_uptake(
                        x, uptake_params["A1"], uptake_params["A2"], uptake_params["B2"]
                    ),
                    0,
                    np.inf,
                )
                ...
                self.update_fit_results(
                    org, uptake_params=uptake_params, area_uptake=area_uptake
                )
    ...
    def num_decays(self, fit_accepted):
        ...
        if "Right Colon" in self.disintegrations.index:
            if pd.isna(self.disintegrations.loc["Right Colon"]).any():
                self.disintegrations.loc["Right Colon"] = self.disintegrations.loc[
                    "Left Colon"
                ]
                self.disintegrations.loc["Rectum"] = self.disintegrations.loc[
                    "Left Colon"
                ]

        if "Red Marrow" in self.disintegrations.index:
            if pd.isna(self.disintegrations.loc["Red Marrow"]).any():
                self.disintegrations.loc["Red Marrow"] = (
                    self.disintegrations.loc["Heart Contents"] * 0.34
                )
    ...
    def calculate_tumor_sink_effect(self):
        ...
        self.tumor_sink_effect_factor = 1 + (
            tumor_value / (wb_value - tumor_value)
        )  # represents a multiplicative adjustment to the organ's disintegration value to account for normalizing or redistributing activity after subtracting the tumor's share from the whole body
    ...
    def phantom_data(self):
        ...
        self.phantom_mass.loc["Remainder Body"] = (
            self.phantom_mass.loc["Body"]
            - self.phantom_mass.loc[self.phantom_mass.index != "Body"].sum()
        )
        largeintestine = ["Left Colon", "Right Colon", "Rectum"]
        self.not_inbiodi = []
        for org in self.phantom_mass.index:
            if (
                org not in self.disintegrations.index
                and org != "Body"
                and org != "Remainder Body"
            ):
    ...
    def remainder_body_uptake(self, tumor_name=None):
        ...
        if "mouse" in self.phantom.lower():
            self.literature_mass.loc["Muscle"] = (
                self.phantom_mass.loc["Remainder Body"] - self.literature_mass.sum()
            )

        self.literature_mass = self.literature_mass.loc[
            self.disintegrations.index.intersection(self.literature_mass.index)
        ]
        print("\nLiterature Mass (g)\n")
        print(self.literature_mass)
        ...
        if "mouse" in self.phantom.lower():
            self.disintegrations["%ID*h"] = (
                self.disintegrations["%ID/g*h"] * self.phantom_mass[self.mouse_mass]
            )
            if "Residual" in self.disintegrations.index:
                self.not_inphantom_notumor.remove("Residual")
                self.disintegrations.loc["Remainder Body", "%ID*h"] = (
                    self.disintegrations["%ID/g*h"]
                    .loc[self.not_inphantom_notumor]
                    .mul(
                        self.literature_mass[self.mouse_mass].loc[
                            self.not_inphantom_notumor
                        ]
                    )
                    .sum()
                ) + (
                    self.disintegrations.loc["Residual", "%ID/g*h"]
                    * (self.phantom_mass.loc["Residual", self.mouse_mass])
                )
            else:
                self.disintegrations.loc["Remainder Body", "%ID*h"] = (
                    self.disintegrations["%ID/g*h"]
            ...
            for org in self.not_inphantom_notumor:
                if org != "Tail":
                    self.disintegrations.loc[org, "%ID*h"] = (
                        self.disintegrations.loc[org, "%ID/g*h"]
                        * self.literature_mass.loc[org, self.mouse_mass]
                    )
                else:
                    pass
            ...
            print("x")
            self.disintegrations["%ID*h Female"] = (
                self.disintegrations["%ID/g*h"] * self.phantom_mass["Female"]
            )
            self.disintegrations["%ID*h Male"] = (
                self.disintegrations["%ID/g*h"] * self.phantom_mass["Male"]
            )

            self.disintegrations.loc["Remainder Body", "%ID*h Female"] = (
                self.disintegrations["%ID/g*h"]
                .loc[self.not_inphantom_notumor]
                .mul(self.literature_mass["Female"])
                .sum()
            )
            self.disintegrations.loc["Remainder Body", "%ID*h Male"] = (
                self.disintegrations["%ID/g*h"]
                .loc[self.not_inphantom_notumor]
                .mul(self.literature_mass["Male"])
                .sum()
            )

        self.disintegrations["h"] = self.disintegrations["%ID*h"] / 100
        self.disintegrations_all_organs = (
            self.disintegrations.copy()
        )  # Store the original disintegrations before dropping organs
        self.disintegrations.drop(
            self.not_inphantom_notumor, inplace=True
    ...
    def calculate_absorbed_dose(self, model, disintegrations):
        ...
        organ_name_map_mouse_olinda = {
            "Large Intestine": "Large Int",
            "Small Intestine": "Small Int",
            "Bladder": "Urin Blad",
            "Remainder Body": "Tot Body",
        }
        organ_name_map_human_olinda = {
            "Gallbladder": "GB Cont",
            "Left Colon": "LLI Cont",
            "Right Colon": "ULI Cont",
            "Stomach": "StomCont",
            "Salivary Glands": "Salivary",
            "Red Marrow": "Red Mar.",
            "Bone Surfaces": "CortBone",
            "Heart": "Hrt Wall",
            "Heart Contents": "HeartCon",
            "Small Intestine": "SI Cont",
            "Bladder": "UB Cont",
            "Remainder Body": "Tot Body",
        }

        if "mouse" in model:
            disintegrations.index = disintegrations.index.to_series().replace(
                organ_name_map_mouse_olinda
            )
        else:
            disintegrations.index = disintegrations.index.to_series().replace(
    ...
    def create_mousecase(
        ...
        input_organs = df.drop(
            set(self.not_inphantom).intersection(set(df.index))
        ).index.to_list()
        ...
        for org in input_organs:  # ignore the tumor here
            ...
            template.iloc[ind[0]] = (
                sourceorgan + "|" + massorgan + "|" + "{:7f}".format(kineticdata)
            )
            template.iloc[ind[uptakepos]] = (
                sourceorgan + "|" + "{:7f}".format(kineticdata)
            )

        now = datetime.datetime.now()
        template.columns = [
            "Saved on " + now.strftime("%m.%d.%Y") + " at " + now.strftime("%H:%M:%S")
        ]
    ...
    def create_human(self, tumor_name=None):
        ...
        if "Small Intestine" not in human.biodi.index:
            ...
            human.disintegrations_all_organs.loc["Small Intestine"] = (
                human.disintegrations_all_organs.loc["Large Intestine"]
            )
        ...
        if "Red Marrow" not in human.disintegrations_all_organs.index:
            try:
                ...
                human.disintegrations_all_organs.loc["Red Marrow", "h Male"] = (
                    human.disintegrations_all_organs.loc["Heart Contents", "h Male"]
                    * 0.34
                )

        if "Tail" in human.disintegrations_all_organs.index:
            print(
                "Tail is not modelled in the human phantom, removing it from the biodi"
            )
            human.biodi = human.biodi.drop("Tail", axis=0)
            human.disintegrations_all_organs = human.disintegrations_all_organs.drop(
                "Tail", axis=0
            )

        if "Tumor" in human.disintegrations_all_organs.index:
            print(
                "Tumor is not modelled in the human phantom, removing it from the biodi"
            )
        ...
        if "h Female" not in human.disintegrations_all_organs:
            human.disintegrations_all_organs.rename(
                columns={"h": "h Male"}, inplace=True
            )
        if "h Female" not in human.disintegrations_all_organs:
            human.disintegrations_all_organs["h Female"] = (
                human.disintegrations_all_organs["h Male"]
            )
        human.disintegrations_all_organs.loc["Rectum", "h Female"] = (
            70 / 360
        ) * human.disintegrations_all_organs.loc["Large Intestine", "h Female"]
        human.disintegrations_all_organs.loc["Rectum", "h Male"] = (
            70 / 370
        ) * human.disintegrations_all_organs.loc["Large Intestine", "h Male"]
        human.disintegrations_all_organs.loc["Left Colon", "h Female"] = (
            145 / 360
        ) * human.disintegrations_all_organs.loc["Large Intestine", "h Female"]
        human.disintegrations_all_organs.loc["Left Colon", "h Male"] = (
            150 / 370
        ) * human.disintegrations_all_organs.loc["Large Intestine", "h Male"]
        human.disintegrations_all_organs.loc["Right Colon", "h Female"] = (
            145 / 360
        ) * human.disintegrations_all_organs.loc["Large Intestine", "h Female"]
        human.disintegrations_all_organs.loc["Right Colon", "h Male"] = (
            150 / 370
        ) * human.disintegrations_all_organs.loc["Large Intestine", "h Male"]
        human.disintegrations_all_organs = human.disintegrations_all_organs.drop(
            "Large Intestine", axis=0
        )
        print(human.disintegrations_all_organs)
        human.not_inphantom = []
        ...
        if tumor_name:
            human.not_inphantom_notumor = [
                org for org in human.not_inphantom if tumor_name not in org
            ]
        else:
            human.not_inphantom_notumor = human.not_inphantom

        human.disintegrations_all_organs.loc["Remainder Body", "h Female"] = sum(
            human.disintegrations_all_organs.loc[
                human.not_inphantom_notumor, "h Female"
            ]
        )
        human.disintegrations_all_organs.loc["Remainder Body", "h Male"] = sum(
            human.disintegrations_all_organs.loc[human.not_inphantom_notumor, "h Male"]
        )

        human.disintegrations_all_organs = human.disintegrations_all_organs[
            ["h Female", "h Male"]
        ]
        human.disintegrations_all_organs.drop(
            human.not_inphantom, inplace=True
        )  # Only organs that are in the phantom will be kept in the disintegrations dataframe and passed to olinda
        human.disintegrations_all_organs.sort_index(inplace=True)
        return human
    ...
    def apply_relative_mass_scaling(self, mouse_mass=25):
        ...
        mouse_mass_sum = rMSF_data.loc[
            self.not_inphantom_notumor, f"{mouse_mass}g_mouse"
        ].sum()
        ...
        remainder_correction_female = (mouse_body_mass / human_body_female) * (
            female_mass_sum / mouse_mass_sum
        )
        remainder_correction_male = (mouse_body_mass / human_body_male) * (
            male_mass_sum / mouse_mass_sum
        )
        ...
        for organ in self.disintegrations_all_organs.index:
            if organ != "Remainder Body":
                ...
                self.disintegrations_all_organs.loc[
                    organ, "h Female"
                ] *= remainder_correction_female
                self.disintegrations_all_organs.loc[
                    organ, "h Male"
    ...
    def create_humancase(self, df, method, savefile=False, dirname="./"):
        ...
        if savefile == True:
            ...
            self.humancase.to_csv(
                dirname + "/" + self.sex + method + ".cas", index=False
            )

        print(
            "The case file {} has been saved in\n{}".format(self.sex + ".cas", dirname)
        )
    ...
    def scale_biexponential_tiac(self, row, biol_lambda_SF=0.25):
        ...
        TIAC_h_bi_male = (
            (Cm_organ_t0_1)
            / (((k_b_male) ** (-1) * lambda_biological1 + lambda_physical))
        ) + (
            (Cm_organ_t0_2)
            / ((k_b_male) ** (-1) * (lambda_biological2) + lambda_physical)
        )

        TIAC_h_bi_female = (
            (Cm_organ_t0_1)
            / (((k_b_female) ** (-1) * lambda_biological1 + lambda_physical))
        ) + (
            (Cm_organ_t0_2)
            / ((k_b_female) ** (-1) * (lambda_biological2) + lambda_physical)
        )
    ...
    def scale_monoexponential_tiac(self, row, biol_lambda_SF=0.25):
        ...
        TIAC_h_mono_male = Cm_organ_t0 / (
            k_b_male ** (-1) * (lambda_biological) + lambda_physical
        )
        TIAC_h_mono_female = Cm_organ_t0 / (
            k_b_female ** (-1) * (lambda_biological) + lambda_physical
        )
    ...
    def lambda_biological_scaling(self, biol_lambda_SF=0.25, tumor_name=None):
        ...
        fit_accepted_df = pd.DataFrame(
            list(self.fit_accepted.items()), columns=["Organ", "fit_accepted"]
        )
        self.fitting_parameters = self.fitting_parameters.merge(
            fit_accepted_df, on="Organ"
        )
        self.fitting_parameters.set_index("Organ", inplace=True)

        if "mouse" in self.phantom.lower():
            self.fitting_parameters["mono_exp:%ID"] = (
                self.fitting_parameters["mono_exp:%ID/g"]
                * self.phantom_mass[self.mouse_mass]
            )
            self.fitting_parameters["bi_exp1:%ID"] = (
                self.fitting_parameters["bi_exp:1_%ID/g"]
                * self.phantom_mass[self.mouse_mass]
            )
            self.fitting_parameters["bi_exp2:%ID"] = (
                self.fitting_parameters["bi_exp:2_%ID/g"]
                * self.phantom_mass[self.mouse_mass]
            )
            if "Residual" in self.fitting_parameters.index:
                self.not_inphantom_notumor.remove("Residual")
                self.fitting_parameters.loc["Remainder Body", "mono_exp:%ID"] = (
                    self.fitting_parameters["mono_exp:%ID/g"]
                    .loc[self.not_inphantom_notumor]
                    .mul(
                        self.literature_mass[self.mouse_mass].loc[
                ...
                self.fitting_parameters.loc["Remainder Body", "mono_exp:%ID"] = (
                    self.fitting_parameters["mono_exp:%ID/g"]
                    .loc[self.not_inphantom_notumor]
                    .mul(self.literature_mass[self.mouse_mass])
                    .sum()
                )
                self.fitting_parameters.loc["Remainder Body", "bi_exp1:%ID"] = (
                    self.fitting_parameters["bi_exp:1_%ID/g"]
                    .loc[self.not_inphantom_notumor]
                    .mul(self.literature_mass[self.mouse_mass])
                    .sum()
                )
                self.fitting_parameters.loc["Remainder Body", "bi_exp2:%ID"] = (
                    self.fitting_parameters["bi_exp:2_%ID/g"]
                    .loc[self.not_inphantom_notumor]
                    .mul(self.literature_mass[self.mouse_mass])
                    .sum()
            ...
            for org in self.not_inphantom_notumor:
                if org != "Tail":
                    print(org)
                    self.fitting_parameters.loc[org, "mono_exp:%ID"] = (
                        self.fitting_parameters.loc[org, "mono_exp:%ID/g"]
                        * self.literature_mass.loc[org, self.mouse_mass]
                    )
                    self.fitting_parameters.loc[org, "bi_exp1:%ID"] = (
                        self.fitting_parameters.loc[org, "bi_exp:1_%ID/g"]
                        * self.literature_mass.loc[org, self.mouse_mass]
                    )
                    self.fitting_parameters.loc[org, "bi_exp2:%ID"] = (
                        self.fitting_parameters.loc[org, "bi_exp:2_%ID/g"]
                        * self.literature_mass.loc[org, self.mouse_mass]
                    )
                else:
                    pass
        ...
        for i, organ in enumerate(self.fitting_parameters.index):
            if self.fitting_parameters.loc[organ, "fit_accepted"] == 1.0:
                TIAC_h_mono_male, TIAC_h_mono_female = self.scale_monoexponential_tiac(
                ...
                TIAC_h_bi_male, TIAC_h_bi_female = self.scale_biexponential_tiac(
                    self.fitting_parameters.iloc[i], biol_lambda_SF
                )
                self.fitting_parameters.loc[organ, "h Male"] = TIAC_h_bi_male
                self.fitting_parameters.loc[organ, "h Female"] = TIAC_h_bi_female

        self.disintegrations_all_organs = self.fitting_parameters[
            ["h Female", "h Male"]
        ]
