...
from pytheranostics.plots.plots import ewin_montage
...
class GammaCamera(PlanarQC):
    ...
    def get_sensitivity(self, source_id="C", **kwargs):
        ...
        if "site_id" in kwargs:
            ...
            df2 = df[
                (df.site_id == kwargs["site_id"])
                & (df.source_id == source_id)
                & (df.cal_type == "planar")
                & (df.model == camera_model)
            ]
            ...
            if "." in acq_time:
                acq_time = np.datetime64(
                    datetime.strptime(f"{acq_date} {acq_time}", "%Y%m%d %H%M%S.%f")
                )
            else:
                acq_time = np.datetime64(
                    datetime.strptime(f"{acq_date} {acq_time}", "%Y%m%d %H%M%S")
                )
            ...
            print(
                f"The activity of the source at the time of the scan was {self.A_decayed} MBq\n"
            )
        ...
        for w in range(nwin):
            lower = (
                self.ds.EnergyWindowInformationSequence[w]
        ...
        sensitivity = {
            k: v / (self.A_decayed * acq_duration) for k, v in self.Cp.items()
        }
        ...
        self.cal_dic[kwargs["site_id"]][camera_model]["manufacturer"] = (
            df2.manufacturer.values[0]
        )
        self.cal_dic[kwargs["site_id"]][camera_model]["collimator"] = (
            df2.collimator.values[0]
        )
        ...
        print("\n")
        print(
            "Calibration Results. Sensitivity in cps/MBq. Calibration factor in MBq/cps"
        )
    ...
    def calculate_uncertainty(self, site_id, camera_model, uncertainty_activity):
        ...
        for detector in ["Detector1", "Detector2"]:
            ...
            u_sc = math.sqrt(
                self.win_check["low_scatter"]["counts"][detector]
                + self.win_check["upper_scatter"]["counts"][detector]
            )
            u_prim = math.sqrt(u_pw**2 + u_sc**2)
            u_prim_list.append(u_prim)

        final_u_counts = 1 / 2 * math.sqrt(
            u_prim_list[0] ** 2 + u_prim_list[1] ** 2
        ) + np.std(u_prim_list)
        counts = (self.Cp["Detector1"] + self.Cp["Detector2"]) / 2
        u_activity = self.A_decayed * uncertainty_activity
        u_time = 0.001

        uncertainty_cf = self.cal_dic[site_id][camera_model]["calibration_factor"][
            "Average"
        ] * math.sqrt(
            (final_u_counts / counts) ** 2
            + (u_activity / self.A_decayed) ** 2
            + (u_time / self.ds.ActualFrameDuration / 1000) ** 2
        )
        uncertainty_sensitivity = self.cal_dic[site_id][camera_model]["sensitivity"][
            "Average"
        ] * math.sqrt(
            (final_u_counts / counts) ** 2
            + (u_activity / self.A_decayed) ** 2
